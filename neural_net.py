...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    leftIntersection = max(scores[0], y[0])
    rightIntersection = min(scores[0] + scores[2], y[0] + y[2])
    topIntersection = min[scores[1], y[1]]
    bottomIntersection = max(scores[1] + scores[3], y[1] + y[3])
    if (left < right and top < bottom):
        intersection = (right - left) * (bottom - top)
        loss = ((y[2] * y[3]) - intersection)
    else:
        loss = (y[2] * y[3])
    #############################################################################
    #                              END OF YOUR CODE                             #
